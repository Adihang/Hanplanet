...
- Google 연동 데이터: Google 사용자 ID, 이메일, 표시 이름, 아바타 URL, OAuth access token, refresh token, 토큰 만료 시각, 권한 범위, Google Drive 표시 허용 여부
- Google Drive 데이터: 사용자가 Google Drive 표시를 켠 경우, Drive 파일/폴더 ID, 이름, MIME 타입, 크기, 수정 시각, 부모 폴더, 파일 내용, 다운로드/업로드/수정/삭제/이동 결과
...
- 사용자가 Google Drive 표시를 명시적으로 켜고 Google Drive 파일을 열람, 업로드, 수정, 이동, 삭제하는 과정
...
- 사용자가 허용한 Google Drive 파일을 HanDrive 안에서 표시하고 읽기, 수정, 생성, 삭제, 이동할 수 있도록 제공
...
서비스는 Google OAuth와 Google Drive API를 사용합니다. Google 계정 연동 시 기본 프로필 정보와 이메일 주소를 사용하여 로그인, 회원가입, 계정 연동을 처리합니다. 사용자가 계정 모달에서 Google Drive 표시 옵션을 켠 경우에만 Google Drive API를 호출하여 Drive 파일을 HanDrive 안에 표시하고, 사용자의 명시적 조작에 따라 파일 읽기, 업로드, 생성, 수정, 이름 변경, 이동, 삭제를 수행합니다.

Google Drive 표시 옵션을 끄면 서버는 Google Drive 관련 HanDrive API 접근을 차단합니다. Google 계정 연동을 해제하면 서비스에 저장된 Google 연동 정보와 토큰은 삭제되며, Google 계정 보안 설정에서 앱 권한을 별도로 철회할 수 있습니다.
...
Google Drive 파일은 기본적으로 Google Drive에 보관되며, 서비스는 사용자의 요청을 처리하기 위해 필요한 범위에서 파일 메타데이터와 파일 내용을 전송하거나 일시적으로 처리할 수 있습니다. 사용자가 Google Drive 파일을 HanDrive로 드래그 앤 드롭하여 업로드하면 해당 파일 사본이 HanDrive 저장소에 저장될 수 있습니다.
...
- Google: Google 로그인, Google 계정 연동, Google Drive 파일 표시 및 조작
...
- Google Drive 표시 옵션 비활성화 시 관련 API 차단
...
- Google integration data: Google user ID, email, display name, avatar URL, OAuth access token, refresh token, token expiration, scopes, and Google Drive display setting
- Google Drive data: when enabled by the user, Drive file/folder IDs, names, MIME types, sizes, modification times, parent folders, file content, and results of download/upload/create/update/delete/move actions
...
- Enable Google Drive display and view, upload, edit, move, or delete Google Drive files
...
- Display and manage connected Google Drive files in HanDrive only when authorized by the user
...
The Service uses Google OAuth and the Google Drive API. Google profile and email information are used for login, sign-up, and account linking. Google Drive API calls are made only when the user enables the Google Drive display option in the account modal. When enabled, the Service displays Drive files inside HanDrive and performs read, upload, create, update, rename, move, and delete actions only in response to user actions.

When Google Drive display is disabled, the server blocks Google Drive-related HanDrive API access. When a Google account is unlinked, locally stored Google connection data and tokens are deleted. Users may also revoke app access in their Google account security settings.
...
Google Drive files remain in Google Drive by default. The Service may transmit or temporarily process file metadata and file content only as needed to fulfill user-requested actions. If a user drags and drops a Google Drive file into HanDrive, a copy of that file may be stored in HanDrive storage.
...
- Google: Google login, Google account linking, and Google Drive file display/management
...
- Disabling Google Drive display
- Account deletion
...
- Server-side blocking of Google Drive APIs when Google Drive display is disabled
