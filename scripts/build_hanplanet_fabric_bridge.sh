...
BRIDGE_DIR="$REPO_DIR/minecraft-fabric-bridge"
BUILD_DIR="$BRIDGE_DIR/build"
CLASSES_DIR="$BUILD_DIR/classes"
JAR_PATH="$BUILD_DIR/hanplanet-fabric-bridge-1.0.0.jar"
...
"$JAVA_HOME/bin/javac" -encoding UTF-8 -cp "$CLASSPATH" -d "$CLASSES_DIR" \
  "$BRIDGE_DIR/src/main/java/dev/hanplanet/minecraft/HanplanetFabricBridge.java"
cp "$BRIDGE_DIR/src/main/resources/fabric.mod.json" "$CLASSES_DIR/fabric.mod.json"
"$JAVA_HOME/bin/jar" --create --file "$JAR_PATH" -C "$CLASSES_DIR" .
mkdir -p "$SERVER_DIR/mods"
cp "$JAR_PATH" "$SERVER_DIR/mods/hanplanet-fabric-bridge-1.0.0.jar"
echo "$SERVER_DIR/mods/hanplanet-fabric-bridge-1.0.0.jar"
