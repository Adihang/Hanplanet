(function () {
    ...
    if (!window.__handriveEditorCompletionMap) {
        window.__handriveEditorCompletionMap = {
            ...
            ".sql": [
                { trigger: "select", insertText: "SELECT *\nFROM table_name\nWHERE condition;", label: "SELECT ... FROM", cursorBack: 24, priority: 100 },
                { trigger: "selectfrom", insertText: "SELECT column_name\nFROM table_name;", label: "SELECT columns", cursorBack: 23, priority: 99 },
                { trigger: "where", insertText: "WHERE condition", label: "WHERE", cursorBack: 9, priority: 98 },
                { trigger: "join", insertText: "JOIN other_table ON other_table.id = table_name.other_id", label: "JOIN ... ON", cursorBack: 50, priority: 97 },
                { trigger: "leftjoin", insertText: "LEFT JOIN other_table ON other_table.id = table_name.other_id", label: "LEFT JOIN ... ON", cursorBack: 50, priority: 96 },
                { trigger: "insert", insertText: "INSERT INTO table_name (column_name)\nVALUES (value);", label: "INSERT INTO", cursorBack: 35, priority: 95 },
                { trigger: "update", insertText: "UPDATE table_name\nSET column_name = value\nWHERE condition;", label: "UPDATE ... SET", cursorBack: 20, priority: 94 },
                { trigger: "delete", insertText: "DELETE FROM table_name\nWHERE condition;", label: "DELETE FROM", cursorBack: 10, priority: 93 },
                { trigger: "create", insertText: "CREATE TABLE table_name (\n    id INTEGER PRIMARY KEY,\n    name TEXT NOT NULL\n);", label: "CREATE TABLE", cursorBack: 59, priority: 92 },
                { trigger: "alter", insertText: "ALTER TABLE table_name\nADD COLUMN column_name TEXT;", label: "ALTER TABLE", cursorBack: 27, priority: 91 },
                { trigger: "index", insertText: "CREATE INDEX index_name ON table_name (column_name);", label: "CREATE INDEX", cursorBack: 37, priority: 90 },
                { trigger: "with", insertText: "WITH cte_name AS (\n    SELECT *\n    FROM table_name\n)\nSELECT *\nFROM cte_name;", label: "WITH CTE", cursorBack: 60, priority: 89 },
                { trigger: "case", insertText: "CASE\n    WHEN condition THEN result\n    ELSE fallback\nEND", label: "CASE WHEN", cursorBack: 42, priority: 88 },
                { trigger: "group", insertText: "GROUP BY column_name", label: "GROUP BY", cursorBack: 11, priority: 87 },
                { trigger: "order", insertText: "ORDER BY column_name ASC", label: "ORDER BY", cursorBack: 15, priority: 86 },
                { trigger: "limit", insertText: "LIMIT 100", label: "LIMIT", priority: 85 },
                { trigger: "transaction", insertText: "BEGIN;\n\nCOMMIT;", label: "BEGIN / COMMIT", cursorBack: 8, priority: 84 },
                { trigger: "count", insertText: "COUNT(*)", label: "COUNT(*)", priority: 83 },
                { trigger: "coalesce", insertText: "COALESCE(value, fallback)", label: "COALESCE", cursorBack: 10, priority: 82 },
                { trigger: "comment", insertText: "-- ", label: "-- comment", priority: 81 },
            ],
            ".html": [
